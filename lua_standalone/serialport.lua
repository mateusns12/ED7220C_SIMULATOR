...
function SerialPort:open()
	...
	if res then
		...
		print(f("\x1b[1;31m[ACHTUNG]\x1b[0m Cannot create: '%s' is not a valid port",msg))
	end
	return self.opened
end
...
function SerialPort:write(msg)
	if self.opened then
		...
		print("\x1b[1;31m[ACHTUNG]\x1b[0m Cannot write: Port isnt Opened")
		return false
	end
end
...
function SerialPort:read()
	if self.opened then
		...
		print("\x1b[1;31m[ACHTUNG]\x1b[0m Cannot read: Port isnt Opened")
		return false,{}
	end
end
...
--[[
--	@param (int delay) Espera um tempo 'delay' em millisegundos
--]]
function SerialPort:wait(delay)
	serial.Wait(delay)
end
